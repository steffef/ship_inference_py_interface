# A convertion of the insert_ais.cpp function to python, using interface generated by pybind11
...
def readFileToVecs(filename):
    """Reads csv file of processed AIS data, and converts it int vectors

    Args:
        filename (_type_): path to .csv file
    ...
    with open(filename_open, 'r') as file:
        ...
        return mmsi_vec, time_vec, x_vec, y_vec, sog_vec, cog_vec

def vecsToShipStateVectorMap(num_ships: int, mmsi_vec: list[int], \
                             time_vec: list[float], x_vec: list[float], \
                             y_vec: list[float], sog_vec: list[float], \
                             cog_vec: list[float]):
    """Converts vectors received from the readFileToVecs() function
    to a list of dictionaries, containing each ship state for every time step

    Args:
        num_ships (int): Total number of ships, including own
        mmsi_vec (list[int]): 
        time_vec: (list[float]):
        x_vec (list[float]):
        y_vec: (list[float]): 
        ...
        cog_vec: (list[float]):
    Returns:
        list[dict[int, np.ndarray]]: ship_state
        list[float]: unique_time_vec
    """

    ship_state: list[dict[int, np.ndarray]] = []
    unique_time_vec: list[float] = []
    ...
    return ship_state, unique_time_vec

def getShiplist(mmsi_vec: list[int]) -> list[int]:
    """ Creates a list of unique mmsi from ais data

    Args:
        mmsi_vec (list[int]): all mmsi readings from an ais file

    Returns:
        list[int]: unique mmsi
    """
...
def getShiplistIndex(mmsi, ship_list):
    """Finds index where in ship list where the given mmsi occurs 
...
def writeIntentionToFile(timestep, parameters, filename, ship_intentions, ship_state, ship_list, unique_time_vec, x_vec, y_vec):
    """Loops over all time steps, calculate intentions for each ship, and writes the intentions to file

    Args:
        timestep (int): time step where the intention models have been initialized
        parameters (IntentionModelParameters):  
        filename (string): path to .csv file
        ship_intentions (dict[int, IntentionModel]): intention models for all ships
        ship_state (dict[int, np.ndarray]): 
        ship_list (list[int]): 
        unique_time_vec (list[float]):
        x_vec (list[float]): 
        y_vec (list[float]): 
...
if __name__ == "__main__":
    ...
    mmsi_vec, time_vec, x_vec, y_vec, sog_vec, cog_vec = readFileToVecs(filename)
    ship_state, unique_time_vec = vecsToShipStateVectorMap(num_ships, mmsi_vec,\
